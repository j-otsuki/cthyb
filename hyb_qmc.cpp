...
static double tau_length(struct cond_op &F);
static double tau_overlap(double, double, struct cond_op &);
		...
		for(int i=0; i<S[s].k; i++){
			for(int j=0; j<S[s].k; j++){
				...
				if(tau>0){
					...
					B->Gf[s][i_tau] -= S[s].mat_M[j][i] * (double)w_sign;
				}
				else{
...
// n > 0
static inline void arrange_1array(struct cond_op &F, double *tau, int *flag_exist, int n)
{
...
int if_double_occup()
{
// 	void arrange_1array(struct cond_op &F, double *tau, int *flag_exist, int n);
...
// 			B->chi_tr0[d_tau] -= S[1].mat_M[j_tau0[ij]][l_tau0[lm]] * S[0].mat_M[m_tau0[lm]][i_tau0[ij]]
// 			                   * (double)(sign_ij0[ij] * sign_lm0[lm]);
			
			double temp_chi_tr = -S[1].mat_M[j_tau0[ij]][l_tau0[lm]] * S[0].mat_M[m_tau0[lm]][i_tau0[ij]]
			                    * (double)(sign_ij0[ij] * sign_lm0[lm]);
...
// 			B->chi_tr1[d_tau] -= S[1].mat_M[j_tau1[ij]][l_tau1[lm]] * S[0].mat_M[m_tau1[lm]][i_tau1[ij]]
// 			                   * (double)(sign_ij1[ij] * sign_lm1[lm]);
			
			double temp_chi_tr = -S[1].mat_M[j_tau1[ij]][l_tau1[lm]] * S[0].mat_M[m_tau1[lm]][i_tau1[ij]]
			                    * (double)(sign_ij1[ij] * sign_lm1[lm]);
...
// 			B->chi_tr[d_tau] -= S[1].mat_M[j_tau[ij]][l_tau[lm]] * S[0].mat_M[m_tau[lm]][i_tau[ij]]
// 			                   * (double)(sign_ij[ij] * sign_lm[lm]);
			
			double temp_chi_tr = -S[1].mat_M[j_tau[ij]][l_tau[lm]] * S[0].mat_M[m_tau[lm]][i_tau[ij]]
			                   * (double)(sign_ij[ij] * sign_lm[lm]);
...
static void add_segment0(int sigma)
{
	struct cond_op *F = &S[sigma];
	
	if(F->flag==0)  return;  // |1>
	
// 	struct cond_op *F_ref[N_S-1];
...
static void add_tau(struct cond_op &F, int i_tau1, double tau1, int i_tau2, double tau2)
{
...
static void rem_tau(struct cond_op &F, int i_tau1, int i_tau2)
{
...
// tau_ins : creation
// i_tau_ins is stored
static int reject_create_seg(double *F_tau1, double *F_tau2, int k, int wind, double tau_ins, int &i_tau_ins, double &l_max)
{
...
// 				double l_over = tau_overlap(F->tau2[i_tau2], F->tau1[i_tau1], S[s_ref[r]]);
	...
	double lambda = F->mat_M[i_tau1][i_tau2];
	...
	if( metropolis_abs(fabs(prob), ACCPT_sub.n_accept_seg, ACCPT_sub.n_reject_seg) ){
		...
		// update tau-array
		//
// 		void remove_tau(struct cond_op &F, int i_tau1, int i_tau2);
...
// 	double prob = sgn(tau1-tau2) * lambda * b_fac * prm.beta * l_max / (double)(F->k+1);
	...
	if( metropolis_abs(fabs(prob), ACCPT_sub.n_accept_seg, ACCPT_sub.n_reject_seg) ){
		...
		// update tau-array
		//
// 		void add_tau(struct cond_op &F, int, double, int, double);
		add_tau(*F, i_tau1, tau1, i_tau2, tau2);
...
static void remove_tau(struct cond_op &F, int i_tau1, int i_tau2)
{
		for(int i=i_tau1; i<F.k-1; i++)  F.tau1[i] = F.tau1[i+1];
		for(int i=i_tau2; i<F.k-1; i++)  F.tau2[i] = F.tau2[i+1];
		F.tau1[F.k-1] = F.tau1[0] + prm.beta;
...
static void shift_tau2_all(struct cond_op *S, int sigma)
{
	...
	if( metropolis_abs(fabs(prob), ACCPT_sub.n_accept_shift, ACCPT_sub.n_reject_shift) ){
		...
		if( flag_change==1 ){
			if( F->flag ){  // flag==1: tau1[i] > tau2[i],  rotate downward
				...
				rotate_columns_downward(F->mat_M, F->k);
			}
			else{  // flag==1: tau1[i] > tau2[i],  rotate upward
				rotate_upward(F->tau2, F->k);
				rotate_columns_upward(F->mat_M, F->k);
			}
		}
...
// static int tau_overlap(double tau_from, double tau_to, struct cond_op &F, double *l_over, double *l_total)
static double tau_overlap(double tau_from, double tau_to, struct cond_op &F)
{
	if( F.k == 0 ){
		double l_over = 0;
		if( F.flag == 0 ){  // |1>
